...
const _timeSharing=(everydatas)=>{
    ...
    function splitedata (datas) {
        ...
        datas.forEach(function(value, index, arr) {
            timedata.push(value.tradeDate+value.updateTime);
            voldata.push(value.volume);
            pricedata.push(value.nowPrice);
            avgdata.push(value.averagePrice);
        })
    ...
    return {
        backgroundColor: sessionStorage.color,
        title: {
            text: '',
            left: 0
        },
    ...
    function splitData (rawData) {
        ...
        rawData.forEach(function(value, index, arr) {
            ...
            if(value==''){
                ...
                times.push(value.tradeDate+value.updateTime);
            }
            vols.push(value.volume);
        })
        return {
            datas: datas,
            times: times,
